...
|04| Página solicitações do executor| Como executor quero que uma solicitação de serviço seja atribuída a mim por um esquema de distribuição que seja justo e que não aconteça a atribuição de uma mesma solicitação para outro executor, para que consiga ver a solicitação sem que ocorra uma repetição. | média | RF 05 e RF 06 | 1 e 2 |
...
|07| Página avaliações| Como executor quero que ao terminar a solicitação, que o usuário responda uma avaliação para ver se o serviço foi bem executado. | baixa | RF 09 | 3 |
